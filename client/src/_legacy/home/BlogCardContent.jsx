...
const BlogCardContent = ({ blog, variant }) => {
  return (
    <div className="absolute inset-0 flex flex-col justify-between bg-gradient-to-t from-slate-950/95 via-slate-950/30 to-transparent p-6">
      <div>
        <span className="inline-flex rounded-full border border-white/20 bg-[#0a2955]/35 px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.22em] text-white backdrop-blur-md">
          {blog.category}
        </span>
      ...
      <div>
        ...
        <div className="mt-5 flex items-center gap-2 text-sm font-semibold text-[#79ffe0] opacity-0 -translate-x-3 transition-all duration-300 group-hover:translate-x-0 group-hover:opacity-100 group-focus-within:translate-x-0 group-focus-within:opacity-100">
          Read Article
          <ArrowRight size={16} />
        </div>
